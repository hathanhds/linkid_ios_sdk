...
class GiftConfirmExchangeViewController: BaseViewController {
    ...
    func setUpGiftItem() {
        ...
        giftInfoContainerView.layer.borderColor = UIColor.cEFEFF6?.cgColor
    }
            ...
            case .success(let res):
                ...
                if (isOtpSent) {
                    ...
                    self.navigator.show(segue: .giftExchangeSuccess(giftInfo: viewModel.giftInfo,
                        transactionInfo: res?.items?.first ?? CreateTransactionItem(), quantiry: viewModel.output.quantity.value)) { [weak self] vc in
                        self?.navigationController?.pushViewController(vc, animated: true)
                    }
                }
                break

    ...
    init(userRepository: UserRepository,
        ...
        self.viewDidLoadSubj.subscribe { [weak self] _ in
            ...
            if let data = receiverInfoModel {
                ...
                selectedCitySubj.accept(data.city)
                selectedDistrictSubj.accept(data.district)
                selectedWardSubj.accept(data.ward)
...
extension PhysicalShippingViewModel {
    ...
    func validatePhone(_ phone: String?) {
        if (phone ?? "").trim().isEmpty {
            phoneErrorTextSubj.accept("Số điện thoại không được bỏ trống")
        } else if (!isValidPhoneNumber(phone ?? "")) {
            phoneErrorTextSubj.accept("Số điện thoại không đúng định dạng")
        } else {
            phoneErrorTextSubj.accept(nil)
        }
        checkEnableButton()
    }

...
class GiftDetailDiamondViewController: BaseViewController {
    ...
    class func create(with navigator: Navigator, viewModel: GiftDetailDiamondViewModel) -> Self {
        let vc = UIStoryboard.gifts.instantiateViewController(ofType: GiftDetailDiamondViewController.self)
    ...
    override func viewDidLayoutSubviews() {
        ...
        exchangeButton.setGradient(colors: [.c92653E!, .cD4A666!, .c92653E!], direction: .right)
        floatNavView.setGradient(colors: [.c0F0F0F!, .c47372B!], direction: .right)
        self.view.setGradient(colors: [.c0F0F0F!, .c47372B!], direction: .down)
    }
    ...
    func setUpGiftPirce(_ giftInfor: GiftInfor) {
        ...
        if let discountPrice = giftInfor.discountPrice, discountPrice > 0 {
            discountCoinLabel.text = "-\(discountPrice)%"
    ...
    func setUpGeneralInfo(_ giftInfor: GiftInfor) {
        ...
        // Điều kiện áp dụng
        if let condition = giftInfor.condition {

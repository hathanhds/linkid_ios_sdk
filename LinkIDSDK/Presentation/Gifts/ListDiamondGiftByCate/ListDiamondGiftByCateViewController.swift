...
class ListDiamondGiftByCateViewController: BaseViewController, ViewControllerType {
    ...
    static func create(with navigator: Navigator, viewModel: ListDiamondGiftByCateViewModel) -> Self {
        let vc = UIStoryboard.gifts.instantiateViewController(ofType: ListDiamondGiftByCateViewController.self)
    ...
    override func initView() {
        ...
        giftCollectionView.register(UINib(nibName: String(describing: DiamondGiftCollectionViewHeader.self), bundle: UIStoryboard.bundle), forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "DiamondGiftHeaderViewIdentifier")
        giftCollectionView.register(UINib(nibName: String(describing: LoadMoreFooterCollectionView.self), bundle: UIStoryboard.bundle), forSupplementaryViewOfKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: "LoadMoreFooterCollectionViewIdentifier")
        giftCollectionView.registerCellFromNib(ofType:
                DiamondGiftCollectionViewCell.self)
        giftCollectionView.addPullToRefresh(target: self, action: #selector(self.onRefresh), tintColor: .cE9B161)
...
//        layout.minimumInteritemSpacing = 16
    ...
    func setupImagetViewBehindBars() {
        ...
        let tabmanBarHeight: CGFloat = 200 // Adjust if you have a custom size for Tabman bar
        ...
        navView = UIView(frame: CGRect(x: 0, y: 0, width: view.bounds.width, height: statusBarHeight + navigationBarHeight + tabmanBarHeight))
        navImageView = UIImageView(frame: navView!.bounds)
...
extension ListDiamondGiftByCateViewController: SkeletonCollectionViewDataSource, SkeletonCollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if collectionView == giftCollectionView {
            if kind == UICollectionView.elementKindSectionHeader {
                let header: DiamondGiftCollectionViewHeader = collectionView.dequeueHeaderFooterView(ofKind: kind, withIdentifier: "DiamondGiftHeaderViewIdentifier", for: indexPath)
                // Configure your header view here
                header.setDataForCell(data: viewModel.output.selectedCate.value!)
                return header
            } else if (kind == UICollectionView.elementKindSectionFooter) {
                let footer: LoadMoreFooterCollectionView = collectionView.dequeueHeaderFooterView(ofKind: kind, withIdentifier: "LoadMoreFooterCollectionViewIdentifier", for: indexPath)
        ...
        if(collectionView == giftCollectionView) {
            if let descTitle = viewModel.output.selectedCate.value!.descTitle, let descContent = viewModel.output.selectedCate.value!.descContent {
                if(descTitle.count + descContent.count == 0) {
                    ...
                    let label1Height = descTitle.count > 0 ? UtilHelper.heightForLabel(text: descTitle, font: UIFont(name: "BeVietnamPro-SemiBold", size: 14.0)!, width: contentWidth) : 0
                    let label2Height = descContent.count > 0 ? UtilHelper.heightForLabel(text: descContent, font: UIFont(name: "BeVietnamPro-Regular", size: 12.0)!, width: contentWidth) : 0
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if(collectionView == cateCollectionView) {
            ...
            let gift = viewModel.output.gifts.value[indexPath.row]
            self.navigator.show(segue: .giftDetailDiamond(giftInfo: gift)) { [weak self] vc in
                guard let self = self else { return }
